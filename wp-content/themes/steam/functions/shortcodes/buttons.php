...
class itButtons {
	...
	public static function button( $atts = null, $content = null ) {
		...
		if($type=='link') {
			...
			$out = '<a href="' . esc_url( $link ) . '" class="btn ' . $size . ' ' . $variation . ' ' . $block . '" ' . $target . '>' . it_remove_wpautop( $content ) . '</a>';
			
		} else {
			
			$out = '<button class="btn ' . $size . ' ' . $variation . ' ' . $block . '" type="button">' . it_remove_wpautop( $content ) . '</button>';
