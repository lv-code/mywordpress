...
var alliAdmin = {
	...
	init : function() {
		...
		alliAdmin.taxonomiesConfirm();
		alliAdmin.minisitesConfirm();
	...
	minisitesConfirm : function () {
		jQuery('.it_confirm_minisites').click(function(e){			
			if (confirm(objectL10n.minisitesConfirm)){
				jQuery('#it_full_submit').val(1);
			} else {
				e.preventDefault();
			}
	...
	taxonomiesConfirm : function () {
		jQuery('.it_confirm_taxonomies').click(function(e){			
			if (confirm(objectL10n.taxonomiesConfirm)){
				jQuery('#it_full_submit').val(1);
			} else {
				e.preventDefault();
			}
	...
	tooltipHelp : function() {
		...
		jQuery('body').on('mouseover', '.it_option_help a', function(){
		   if (!jQuery(this).hasClass("tooledUp")){
		      jQuery(this).tooltip({ delay: 150, predelay: 0, effect: 'slide', relative: true, direction: 'left', offset: [-4, -138], opacity: 0.9, relative: true, tipClass: 'it_help_tooltip' });
