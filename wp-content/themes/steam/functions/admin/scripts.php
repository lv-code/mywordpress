...
function it_admin_enqueue_scripts( $hook ) {
	...
	if ( in_array( $hook,  array( 'appearance_page_it-options' ) ) ) {
		...
		wp_enqueue_style( IT_PREFIX . '-admin-menu', THEME_ADMIN_ASSETS_URI . '/css/menu.css', false, THEME_VERSION, 'screen' );
		...
		wp_localize_script( IT_PREFIX . '-admin-js', 'objectL10n', array(
			...
			'minisitesConfirm' => __( 'This will save all options and update your list of minisites at the bottom of the control panel menu on the left. Continue?', IT_TEXTDOMAIN ),
			'taxonomiesConfirm' => __( 'This will save all options and update your taxonomies for all of your minisites. Continue?', IT_TEXTDOMAIN ),
...
function it_admin_print_scripts() {
	...
	jQuery(document).ready(function(){
		//itAdmin.menuSort(); /*altered by industrialthemes*/
	});
	//]]>\r</script>\r";
}
