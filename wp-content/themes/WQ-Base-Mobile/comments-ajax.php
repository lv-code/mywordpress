...
if ( $user->ID ) {
	...
	if ( get_option('comment_registration') || 'private' == $status )
		err(__('Sorry,需要登陆才能发票评论.')); // 將 wp_die 改為錯誤提示
}
...
if ( get_option('require_name_email') && !$user->ID ) {
	if ( 6 > strlen($comment_author_email) || '' == $comment_author )
		err( __('Error: 必须填写用户名和邮箱.') ); // 將 wp_die 改為錯誤提示
	elseif ( !is_email($comment_author_email))
		err( __('Error: 必须输入一个有效的邮箱地址.') ); // 將 wp_die 改為錯誤提示
}

if ( '' == $comment_content )
	err( __('Error: 请填写评论内容.') ); // 將 wp_die 改為錯誤提示
