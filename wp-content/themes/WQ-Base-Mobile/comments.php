...
?>
		...
		<div id="commnents" class="commentsorping">
				<div class="commentsays">
						<?php comments_number('暂时没有回复', '现在只有1个回复', '已经有%个回复' );?>
				</div>
				<div class="commentpart"><span class="commentparttri"></span>展开评论<?php echo (' (' . (count($comments)-count($trackbacks)) . ')'); ?></div>
				<div class="pingpart" style="color: rgb(153, 153, 153);"><span class="pingparttri"></span>收起评论<?php echo (' (' . count($trackbacks) . ')');?></div>
				<div class="clear"></div>
		</div>
		...
		<div class="commentshow">
				<ol class="commentlist">
						<?php wp_list_comments('type=comment&callback=theme_comments&max_depth=10000'); ?>
				</ol>
		...
		<div id="respond">
				...
				<div class="writerinfodiv">You must be <a href="<?php echo get_option('siteurl'); ?>/wp-login.php?redirect_to=<?php echo urlencode(get_permalink()); ?>">logged in</a> to post a comment.</div>
				<?php else : ?>
				<form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform" name="comm_frm">
						...
						<div class="writerinfodiv"> <?php printf(__('哟，<strong>%s</strong>，欢迎回来！'), $comment_author) ?> <span id="show_author_info"><a href="javascript:setStyleDisplay('author_info','');setStyleDisplay('show_author_info','none');setStyleDisplay('hide_author_info','');">( 换个马甲 )</a></span> <span id="hide_author_info"><a href="javascript:setStyleDisplay('author_info','none');setStyleDisplay('show_author_info','');setStyleDisplay('hide_author_info','none');">( 关闭 )</a></span> </div>
						<?php endif; ?>
						<div id="author_info">
								...
								<div class="writerinfodiv">
										...
										<label for="email" >
												<?php _e('邮箱');?>
												<?php if ($req) _e('(用于显<a href="http://www.gravatar.com" target="_blank">Gravata</a>头像，必填)'); ?>
										</label>
								</div>
								<div class="writerinfodiv">
