...
class CT_Popular_Post extends WP_Widget {
	...
	function  CT_Popular_Post() {
		...
		$widget_ops = array(	'classname'		=> 'ct-popularpost-widget',
								'description' => esc_html__( 'A widget that show popular posts' , 'color-theme-framework' )
					);

		/* Create the widget. */
		parent::__construct( 'ct-popularpost-widget', esc_html__('CT: Popular Posts', 'color-theme-framework'), $widget_ops );
	}
...
<?php if ( $popular_posts->have_posts() ) : ?>
		<ul class="ct-custom-widget popular-posts-widget popular-widget-<?php echo esc_attr( $time_id ); ?>">
	...
	function form($instance)
	{
		...
		$defaults = array(
							'title'					=>	esc_html__( 'Posts' , 'color-theme-framework' ),
							'icon_class' 			=> 'comments',
							'num_posts'				=>	4,
							'categories'			=>	'all',
							'categories_exclude'	=>	'',
	...
	<p>
		...
		<select id="<?php echo $this->get_field_id( 'theme_orderby' ); ?>" name="<?php echo $this->get_field_name( 'theme_orderby' ); ?>" class="widefat" style="width:100%;">
			...
			<option <?php if ( 'date' == $instance['theme_orderby'] ) echo 'selected="selected"'; ?>>date</option>
		</select>
	</p>
