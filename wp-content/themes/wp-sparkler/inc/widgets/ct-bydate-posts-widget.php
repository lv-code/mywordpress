...
-----------------------------------------------------------------------------------
	...
	Plugin Name: CT Posts by Date Widget
	Plugin URI: http://www.color-theme.com
	Description: A widget that show recent posts by the date
	Version: 1.0
	Author: ZERGE
 ...
 */
add_action( 'widgets_init', 'ct_posts_bydate_load_widgets' );

function ct_posts_bydate_load_widgets()
{
	register_widget('CT_Posts_Bydate_Widget');
}
 ...
 *
 */
class CT_Posts_Bydate_Widget extends WP_Widget {

	/**
	 * Widget setup.
	 */
	function CT_Posts_Bydate_Widget()
	{
		/* Widget settings. */
		$widget_ops = array(
								'classname' => 'ct_posts_bydate_widget',
								'description' => esc_html__( 'Display recent posts by the date' , 'color-theme-framework' )
							);

		/* Create the widget. */
		parent::__construct( 'ct_posts_bydate_widget', esc_html__( 'CT: Posts by the Date' , 'color-theme-framework' ), $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$background_title 	= ! empty ( $instance['background_title'] ) ? $instance['background_title'] : '#ee445f';
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['post_filter'] 		= $new_instance['post_filter'];
	...
	function form($instance)
	{
		...
		$defaults = array(
			'title'					=>	esc_html__( 'Last Day' , 'color-theme-framework' ),
			'icon_class' 			=> 'clock-o',
			'num_posts'				=>	4,
			'post_filter'			=> 'last_day',
			'categories'			=>	'all',
			'categories_exclude'	=>	'',
			'show_date' 			=> 'on',
			'show_likes' 			=> 'off',
	...
	<p>
		<label for="<?php echo $this->get_field_id( 'post_filter' ); ?>"><?php esc_html_e('Get posts by:', 'color-theme-framework'); ?></label>
		<select id="<?php echo $this->get_field_id( 'post_filter' ); ?>" name="<?php echo $this->get_field_name( 'post_filter' ); ?>" class="widefat" style="width:100%;">
			<option <?php if ( 'last_day' == $instance['post_filter'] ) echo 'selected="selected"'; ?>>last_day</option>
			<option <?php if ( 'last_week' == $instance['post_filter'] ) echo 'selected="selected"'; ?>>last_week</option>
			<option <?php if ( 'last_month' == $instance['post_filter'] ) echo 'selected="selected"'; ?>>last_month</option>
			<option <?php if ( 'last_year' == $instance['post_filter'] ) echo 'selected="selected"'; ?>>last_year</option>
		</select>
	</p>
	...
	<p>
		<label for="<?php echo $this->get_field_id('categories_exclude'); ?>"><?php esc_html_e( 'Categories to exclude: (Use the CTRL key (PC) or COMMAND key (Mac) to select multiple items.)' , 'color-theme-framework' ); ?></label>
	...
	<p>
		...
		<input class="ct-color-picker" type="text" id="<?php echo $this->get_field_id( 'background_title' ); ?>" name="<?php echo $this->get_field_name( 'background_title' ); ?>" value="<?php echo esc_attr( $instance['background_title'] ); ?>" data-default-color="#ee445f" />
	</p>
